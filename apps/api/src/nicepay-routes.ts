...
type Viewer = { id: string; role: string; name: string; email: string };
type Metadata = { clientId: string; mode: string; phase: string; goodsName: string; tid?: string };
type Dependencies = {
  prisma: PrismaClient | null;
  getUser: (req: express.Request) => Promise<Viewer | undefined>;
  getRate: () => Promise<number>;
  transport?: typeof fetch;
};

export function installNicepay(app: express.Express, deps: Dependencies) {
  ...
  const wrap = (fn: (req: express.Request, res: express.Response) => Promise<unknown>): express.RequestHandler =>
    (req, res) => { void fn(req, res).catch(error => {
  ...
  const ready = () => { if (!config.ready || !db) throw new PaymentError('PG_NOT_READY', 503); };
  ...
  async function findOrder(orderId: string) {
    ...
    if (!order || !stored) throw new PaymentError('ORDER_NOT_FOUND', 404);
    const meta: Metadata = JSON.parse(stored.value);
    if (meta.clientId !== config.clientId || meta.mode !== config.mode) throw new PaymentError('PAYMENT_ENVIRONMENT_MISMATCH', 409);
    return { order, meta };
  }
  async function savePhase(orderId: string, meta: Metadata, phase: string) {
  ...
  async function synchronize(orderId: string) {
    ...
    if (!meta.tid) return order;
    const result = verifyTransaction(await call(`/v1/payments/${encodeURIComponent(meta.tid)}`),
      { tid: meta.tid, orderId, amount: order.pointAmount }, config.secretKey);
    // An unresolved network cancellation must not grant a paid entitlement.
    if (result.status === 'paid' && meta.phase === 'NETWORK_CANCEL_PENDING') return order;
    ...
    await db!.$transaction(async tx => {
      ...
      if (current.status === 'REFUNDED' && status === 'PAID') return;
  ...
  app.post('/api/payments/checkout', wrap(async (req, res) => {
    ...
    if (!parsed.success) throw new PaymentError('INVALID_ORDER');
    const product = await db!.digitalProduct.findUnique({ where: { id: parsed.data.productId }, include: { creator: true } });
    if (!product?.isActive || !product.creator.isActive || ![product.creatorId, product.creator.slug].includes(parsed.data.creatorId)) throw new PaymentError('PRODUCT_UNAVAILABLE', 404);
    if (!Number.isSafeInteger(product.pointPrice) || product.pointPrice <= 0) throw new PaymentError('INVALID_PRODUCT_PRICE');
    ...
    if (config.mode === 'production') {
      ...
      if (approved?.value !== 'true') throw new PaymentError('PRODUCT_NOT_APPROVED', 409);
    }
    const rate = await deps.getRate();
    if (!Number.isFinite(rate) || rate < 0 || rate > 100) throw new PaymentError('INVALID_COMMISSION', 503);
    const adminFee = Math.round(product.pointPrice * rate / 100);
    const orderId = `np_${nanoid(24)}`;
    const meta: Metadata = { clientId: config.clientId, mode: config.mode, phase: 'PENDING', goodsName: product.title };
    await db!.$transaction(async tx => {
    ...
    let goodsName = '';
    for (const character of product.title) { if (Buffer.byteLength(goodsName + character) > 40) break; goodsName += character; }
  ...
  app.post('/api/payments/return', express.urlencoded({ extended: false, limit: '16kb' }), wrap(async (req, res) => {
    ...
    if (auth.amount !== order.pointAmount) throw new PaymentError('PAYMENT_AMOUNT_MISMATCH');
    if (meta.tid && meta.tid !== auth.tid) throw new PaymentError('TRANSACTION_MISMATCH', 409);
    if (!meta.tid) {
      if (order.status !== 'PENDING_PAYMENT' || Date.now() - order.createdAt.getTime() > 30 * 60 * 1000) throw new PaymentError('ORDER_EXPIRED', 409);
      try {
        await db!.$transaction(async tx => {
      ...
      } catch { throw new PaymentError('PAYMENT_ALREADY_PROCESSING', 409); }
      try {
        verifyTransaction(await call(`/v1/payments/${encodeURIComponent(auth.tid)}`, { amount: order.pointAmount }),
          { tid: auth.tid, orderId: auth.orderId, amount: order.pointAmount }, config.secretKey);
      } catch {
  ...
  app.get('/api/payments/status', wrap(async (req, res) => {
    ...
    const { order, meta } = await findOrder(orderId);
    if (order.fanId !== user.id && user.role !== 'ADMIN') throw new PaymentError('FORBIDDEN', 403);
  ...
  app.post('/api/payments/webhook', wrap(async (req, res) => {
    ...
    if (!parsed.success) throw new PaymentError('INVALID_WEBHOOK');
    const { meta } = await findOrder(parsed.data.orderId);
    if (!meta.tid || meta.tid !== parsed.data.tid) throw new PaymentError('TRANSACTION_MISMATCH');
  ...
  app.post('/api/admin/refund', wrap(async (req, res) => {
    ready();
    const user = await viewer(req);
    if (user.role !== 'ADMIN') throw new PaymentError('FORBIDDEN', 403);
    const parsed = z.object({ orderId: z.string().max(64), reason: z.string().min(1).max(30) }).safeParse(req.body);
    if (!parsed.success) throw new PaymentError('INVALID_REFUND');
    const { order, meta } = await findOrder(parsed.data.orderId);
    if (!meta.tid || order.payoutStatus === 'SENT') throw new PaymentError('REFUND_REQUIRES_REVIEW', 409);
    const current = await synchronize(order.orderNo);
    if (current.status === 'REFUNDED') return res.json({ status: 'REFUNDED' });
    if (current.status !== 'PAID') throw new PaymentError('ORDER_NOT_PAID', 409);
    try {
    ...
    } catch { throw new PaymentError('REFUND_REQUIRES_RECONCILIATION', 409); }
    ...
    if (updated.status !== 'REFUNDED') throw new PaymentError('REFUND_REQUIRES_RECONCILIATION', 409);
