...
export function nicepayConfig(env = process.env) {
  ...
  try {
    ...
    if (url.protocol === 'https:' || (mode === 'sandbox' && ['localhost', '127.0.0.1'].includes(url.hostname))) origin = url.origin;
  } catch { /* Configuration is reported as unavailable below. */ }
  const ready = Boolean(origin && secretKey && (
    (mode === 'sandbox' && clientId.startsWith('S2_')) ||
...
export function validateCallback(body: unknown, config: ReturnType<typeof nicepayConfig>) {
  const parsed = callbackSchema.safeParse(body);
  if (!parsed.success) throw new PaymentError('INVALID_PAYMENT_AUTH');
...
export function verifyTransaction(body: unknown, expected: { tid: string; orderId: string; amount: number }, secret: string) {
  const parsed = transactionSchema.safeParse(body);
  if (!parsed.success) throw new PaymentError('PG_RESPONSE_INVALID', 502);
  const data = parsed.data;
...
export function nicepayClient(config: ReturnType<typeof nicepayConfig>, transport: typeof fetch = fetch) {
  return async (path: string, body?: Record<string, unknown>) => {
    if (!config.ready) throw new PaymentError('PG_NOT_READY', 503);
    try {
      ...
      if (!response.ok) throw new PaymentError('PG_REQUEST_FAILED', 502);
