...
async function issueSession(user: User) {
  ...
  } else sessions.set(token, user.id);
...
async function getUserFromRequest(req: express.Request) {
  ...
  if (prisma) {
    ...
    return { id: user.id, name: user.displayName, email: user.email, password: '', role: user.role as UserRole, createdAt: user.createdAt.toISOString() };
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (dbReady()) {
    return seedDatabase().then(async () => {
      ...
      if (!dbUser || !valid) return res.status(401).json({ code: 'INVALID_CREDENTIALS' });
      const user: User = {
        id: dbUser.id,
...
app.post('/api/auth/logout', async (req, res) => {
  ...
  if (prisma && token) await prisma.adminSetting.deleteMany({ where: { key: sessionStorageKey(token) } });
  res.status(204).send();
});
...
app.get('/api/supports', async (_req, res) => {
  ...
  if (!viewer) return res.status(401).json({ code: 'UNAUTHORIZED' });
  if (viewer.role !== 'ADMIN') return res.status(403).json({ code: 'FORBIDDEN' });
  if (dbReady()) {
    await seedDatabase();
